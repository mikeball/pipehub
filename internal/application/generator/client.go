...
// to make everything works: 'go.mod' and 'pipe_dynamic.go'.
//
// The 'go.mod' describe the project that should be imported into the project. 'pipe_dynamic.go'
...
// Place where the 'pipe_dynamic.go' file need to be written.
...
// Being more specific, we relly on 2 files: 'pipe_dynamic.go' and 'go.mod'.
//
// 'pipe_dynamic.go' is used to initialize the pipe and 'go.mod' is used to solve the dependencies.
type Client struct {
	config ClientConfig
}
...
func (c Client) Do() error {
	...
	// Generate the 'pipe_dynamic.go' file.
	if err := c.execPipeDynamicTemplate(tmpl, tmplContent); err != nil {
		return errors.Wrap(err, "template 'pipe_dynamic.go' execution error")
	}
...
func (c Client) execPipeDynamicTemplate(tmpl template.Template, content templateContent) error {
	// Delete the old file if it exists.
	var (
		path = "internal/application/server/pipe_dynamic.go"
