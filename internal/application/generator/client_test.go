...
func TestClientDo(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			templates: []string{
				...
				"template/pipe_dynamic.go.tmpl",
			},
			goMod: "testdata/go.mod.1.input",
			output: map[string]string{
				"go.mod":          "testdata/go.mod.1.output",
				"pipe_dynamic.go": "testdata/pipe_dynamic.go.1.output",
			},
			err: require.NoError,
		},
		{
			...
			output: map[string]string{
				...
				"pipe_dynamic.go": "testdata/pipe_dynamic.go.2.output",
			},
			err: require.NoError,
		},
		{
			...
			output: map[string]string{
				...
				"pipe_dynamic.go": "testdata/pipe_dynamic.go.3.output",
			},
			err: require.NoError,
		},
		{
			...
			output: map[string]string{
				...
				"pipe_dynamic.go": "testdata/pipe_dynamic.go.4.output",
			},
			err: require.NoError,
		},
		{
			...
			output: map[string]string{
				...
				"pipe_dynamic.go": "testdata/pipe_dynamic.go.5.output",
			},
			err: require.NoError,
		},
		{
			...
			output: map[string]string{
				...
				"pipe_dynamic.go": "testdata/pipe_dynamic.go.6.output",
			},
			err: require.NoError,
		},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			checkFilesystem(t, fs, "internal/application/server/pipe_dynamic.go", tt.output["pipe_dynamic.go"])
		})
	}
}
...
func initFilesystemFiles(t *testing.T, fs afero.Fs, origin []string, destination []string) {
	t.Helper()
	...
	for i, o := range origin {
		...
		df, err := fs.OpenFile(destination[i], os.O_RDWR|os.O_CREATE, 0644)
